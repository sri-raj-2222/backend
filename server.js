...
// ─── Gemini AI setup (separate from Groq — uses the GEMINI_API_KEY) ──────────
const { GoogleGenerativeAI } = require('@google/generative-ai');
const geminiClient = process.env.GEMINI_API_KEY
  ? new GoogleGenerativeAI(process.env.GEMINI_API_KEY)
  : null;

async function geminiEvaluateReviews(reviews) {
  if (!geminiClient || !reviews.length) return null;
  const model = geminiClient.getGenerativeModel({ model: 'gemini-1.5-flash' });

  const reviewLines = reviews.map((r, i) =>
    `Review ${i + 1}: [${r.rating}/5 stars] ${r.skill_level ? `(${r.skill_level}) ` : ''}${r.comment || 'No comment'}`
  ).join('\n');

  const avgRaw = reviews.reduce((s, r) => s + (r.rating || 0), 0) / reviews.length;

  const prompt = `You are a professional peer-exchange platform evaluator on ShareSphere.
Below are ALL ${reviews.length} peer review(s) received by a user. Analyze them holistically.

REVIEWS:
${reviewLines}

Compute numerical avg: ${avgRaw.toFixed(2)} / 5.

Return ONLY valid JSON in this exact shape:
{
  "avg_score": <number 1.0–5.0, 1 decimal, reflecting the real average>,
  "sentiment": <"Excellent" | "Great" | "Good" | "Mixed" | "Poor">,
  "summary": "<2–3 sentence professional overall evaluation of this user's peer exchange performance>",
  "highlights": ["<strength 1>", "<strength 2>", "<strength 3 — max 3>"],
  "improvement": "<one short actionable suggestion, or null if all reviews are positive>"
}`;
  ...
  try {
    ...
    console.error('[Gemini Reviews] Error:', e.message);
    return null;
  }
}

// GET /api/user/:userId/reviews
// Returns { reviews: [...], avgRating, reviewCount, aiEvaluation }
// aiEvaluation is generated by Gemini and cached in the user's local-DB profile.
app.get('/api/user/:userId/reviews', async (req, res) => {
  try {
    ...
    const reviews = (dbData.reviews || []).filter(
      r => r.target_user_id === userId || r.reviewed_id === userId
    );
    ...
    // Find (or create) user profile for caching
    let userProfile = (dbData.users || []).find(u => u.id === userId);
    if (!userProfile) {
      userProfile = { id: userId, points: 200, reviews_count: 0, rating: 0, analysis: {} };
      if (!dbData.users) dbData.users = [];
      dbData.users.push(userProfile);
    }

    // Use cached evaluation unless: forced refresh, or review count changed, or no cache exists
    let aiEvaluation = userProfile.ai_review_evaluation || null;
    const cachedCount = userProfile.ai_review_evaluation_count || 0;

    if (reviewCount > 0 && (forceRefresh || !aiEvaluation || cachedCount !== reviewCount)) {
      console.log(`[Reviews] Requesting Gemini evaluation for user ${userId} (${reviewCount} reviews)...`);
      aiEvaluation = await geminiEvaluateReviews(reviews);

      if (aiEvaluation) {
        userProfile.ai_review_evaluation = aiEvaluation;
        userProfile.ai_review_evaluation_count = reviewCount;
        userProfile.rating = aiEvaluation.avg_score ?? avgRating;
        const freshDb = db.readDb();
        const idx = freshDb.users.findIndex(u => u.id === userId);
        if (idx !== -1) freshDb.users[idx] = { ...freshDb.users[idx], ...userProfile };
        else freshDb.users.push(userProfile);
        db.writeDb(freshDb);
        console.log(`[Reviews] Gemini evaluation cached for user ${userId}`);
      }
    }

    res.json({
      reviews,
      avgRating: aiEvaluation?.avg_score ?? avgRating,
      reviewCount,
      aiEvaluation,
    });
  } catch (err) {
